...
HT.Hexagon.prototype.getstate = function() {
    return this.state;
};

HT.Hexagon.prototype.setstate = function(s) {
    this.state = s;
};
...
HT.Hexagon.prototype.draw = function(ctx) {
    ...
    if(this.state) this.floodFill(ctx, 255, 0, 0, 0.8);
    // else this.floodFill(ctx, 0, 32, 255, 0.2);
    if(this.Id)
    {
...
HT.Hexagon.prototype.floodFill = function (ctx, r, g, b, opa) {
    var radius = 70;
    var dx = Math.abs(( this.MidPoint.X - this.x) );
    var dy = Math.abs(( this.MidPoint.Y - this.y) );
    var d = Math.sqrt(dx * dx + dy * dy) / 2;
    ctx.beginPath();
    ctx.arc(this.MidPoint.X, this.MidPoint.Y, d, 0, 2 * Math.PI, false);
